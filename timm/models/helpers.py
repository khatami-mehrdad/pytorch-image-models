...
def resume_checkpoint_state_dict(model, checkpoint : dict, optimizer=None, loss_scaler=None, log_info=True):
    resume_epoch = None
...
def resume_checkpoint(model, checkpoint_path, optimizer=None, loss_scaler=None, log_info=True):
    ...
    if os.path.isfile(checkpoint_path):
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
        resume_epoch = resume_checkpoint_state_dict(model, checkpoint, optimizer, loss_scaler, log_info)
        if log_info:
            if 'epoch' in checkpoint:
