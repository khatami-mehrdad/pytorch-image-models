...
def main():
    ...
    try:
        if args.test_only:
            eval_metrics = validate(model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast, output_dir=output_dir)
            return

        # Mehrdad: LTH
        for layer_name, hook in hooks.items():
            compute_apply_sense_sparsity(hook, 0.98, model, loader_eval, validate_loss_fn, args, amp_autocast=amp_autocast)
